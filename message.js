// © 2026 Alpha - FINAL MESSAGE HANDLER (PUBLIC + BRANDING + ANTIBADWORD ACTIVE)
...
// Load global bad words (static list)
...
// Load group-specific bad words config
...
// Combined check: does the message contain any bad word?
const containsBadWord = (text, chatId) => {
    const config = getGroupBadWordsConfig(chatId);
    if (!config.enabled) return false; // antibadword off

    const globalWords = loadGlobalBadWords();
    const groupWords = config.words || [];
...
module.exports = async (sock, m) => {
    try {
        ...
        if (!m.text.startsWith(prefix)) {
            // ================== ANTIBADWORD CHECK (non-command messages) ==================
            if (m.isGroup && m.text) {
                if (containsBadWord(m.text, m.chat)) {
                    // Try to delete the message if bot is admin
                    if (m.isBotAdmin) {
                        try {
                            await sock.sendMessage(m.chat, { delete: m.key });
                        } catch (e) {
                            console.log("Failed to delete badword message:", e.message);
                        }
                    }
                    // Send a warning with mention
        ...
        // 🔥 React with a random emoji (before executing command)
