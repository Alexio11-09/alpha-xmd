    ...
    execute: async (sock, m, { args, reply }) => {
        try {
            ...
            if (mime.includes("image")) {
                ...
                // FIXED: Proper packname/author format
                await sock.sendMessage(m.chat, {
...
/* ================= TICTACTOE ================= */
{
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        let board = games.tictactoe[gameId].board;
...
/* ================= REMOVEBG ================= */
{
    ...
    category: "tools",
    execute: async (sock, m, { reply }) => {
        reply("⚠️ Remove.bg requires an API key.\n\nGet one at: https://www.remove.bg/api\n\nThen use: .removebg [api-key] (reply to image)");
    }
}

];

// Export games for message handler
