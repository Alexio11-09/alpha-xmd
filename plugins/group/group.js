// © 2026 Alpha - GROUP COMMANDS (FUNNY + GUIDES)
...
// Funny guide that shows the correct usage
...
module.exports = [
    ...
    // new commands (with guides where missing)
    {
        command: "approveall", aliases: ["acceptall"], group: true, admin: true,
        execute: async (sock, m, { reply }) => {
            if (!m.isAdmin) return reply(guide("approveall", ".approveall (no arguments needed)"));
            try {
                const meta = await sock.groupMetadata(m.chat);
    ...
    },
    {
        ...
        execute: async (sock, m, { reply }) => {
            if (!m.isAdmin) return reply(guide("kickinactive", ".kickinactive (no arguments needed)"));
            if (!m.isBotAdmin) return reply(R(badmin));
    ...
    },
    {
        ...
        execute: async (sock, m, { args, reply }) => {
            if (!m.isAdmin) return reply(guide("antibadword", ".antibadword on/off/add/remove/list"));
    ...
    },
    {
        ...
        execute: async (sock, m, { args, reply }) => {
            if (!m.isAdmin) return reply(guide("antiforeign", ".antiforeign +263 / on / off / list"));
            if (!m.isBotAdmin) return reply(R(badmin));
    ...
    },
    {
        ...
        execute: async (sock, m, { args, reply }) => {
            if (!m.isAdmin) return reply(guide("antibot", ".antibot on/off (no args = scan)"));
            if (!m.isBotAdmin) return reply(R(badmin));
            const a = args[0]?.toLowerCase();
