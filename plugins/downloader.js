...
module.exports = [
    ...
    // 2. VIDEO – FIXED USING THE SAME APIs YOU TRUST
    { command: "video", aliases: ["vid","dl","yt"], category: "downloader",
      execute: async (s, m, { args, reply, config }) => {
          ...
          // ---- 1. Try the same three APIs but with their VIDEO endpoints ----
          const videoAPIs = [
              `https://yt-dl.officialhectormanuel.workers.dev/?url=${encodeURIComponent(url)}&type=video`,
              `https://api.douxx.tech/api/youtube/video?url=${encodeURIComponent(url)}`,
              `https://api.lolhuman.xyz/api/ytvideo?apikey=GataDios&url=${encodeURIComponent(url)}`
          ];
          for(let api of videoAPIs) {
              try {
                  const r = await axios.get(api, { timeout:10000 });
                  let vidUrl = null;
                  if(r.data?.status) vidUrl = r.data.video || r.data.media;          // officialhectormanuel
                  else if(r.data?.result) vidUrl = r.data.result.download || r.data.result.link; // douxx
                  else if(r.data?.result?.link) vidUrl = r.data.result.link;       // lolhuman
                  if(vidUrl) {
                      await dl(vidUrl, out);
          ...
          // ---- 2. Fallback to ytdl for YouTube ----
          if(!success && /youtube\.com|youtu\.be/.test(url)) {
              try {
                  const stream = ytdl(url, { filter:'videoandaudio', quality:'18' });
                  const writer = fs.createWriteStream(out);
                  stream.pipe(writer);
                  await new Promise((resolve, reject) => {
                      const timer = setTimeout(() => reject(new Error('ytdl timeout')), 60000);
          ...
          // ---- 3. Last resort: universal video scrapers ----
          if(!success && isLink) {
              const apis = [
          ...
          if(success) {
              ...
              reply("❌ Video download failed. The video may be too large, private, or unsupported.");
          }
      }
    },
    ...
    // 9‑12. Social downloaders (FB, IG, MEDIAFIRE, TWITTER) unchanged
    { command: "fb", aliases: ["facebook","fbdl"], category: "downloader",
      execute: async (s, m, { args, reply, config }) => {
          ...
          for(let api of [`https://api.nyxs.pw/dl/fb?url=${encodeURIComponent(url)}`, `https://api.lolhuman.xyz/api/fb?apikey=GataDios&url=${encodeURIComponent(url)}`]) {
              try {
                  const r = await axios.get(api, { timeout:15000 });
                  let vid = r.data?.result?.url || r.data?.url;
                  if(vid) { await dl(vid, out); const ctx=brand(config); await s.sendMessage(m.chat, { video:fs.readFileSync(out), caption:"✅ Downloaded from Facebook", contextInfo:ctx }, { quoted:m }); cleanup(out); return; }
              } catch {}
          }
    ...
    { command: "ig", aliases: ["insta","instagram","igdl"], category: "downloader",
      execute: async (s, m, { args, reply, config }) => {
          ...
          for(let api of [`https://api.nyxs.pw/dl/ig?url=${encodeURIComponent(url)}`, `https://api.lolhuman.xyz/api/instagram?apikey=GataDios&url=${encodeURIComponent(url)}`]) {
              try {
                  const r = await axios.get(api, { timeout:15000 });
                  ...
                  if(media) {
                      ...
                      await dl(media, fp); const ctx=brand(config);
    ...
    { command: "mediafire", aliases: ["mf","mfdl"], category: "downloader",
      execute: async (s, m, { args, reply, config }) => {
          ...
          try {
              ...
              const fp = out+'_'+fn; await dl(dlUrl, fp); const ctx=brand(config);
    ...
    { command: "twitter", aliases: ["x","tw","tweet"], category: "downloader",
      execute: async (s, m, { args, reply, config }) => {
          ...
          for(let api of [`https://api.nyxs.pw/dl/twitter?url=${encodeURIComponent(url)}`, `https://api.lolhuman.xyz/api/twitter?apikey=GataDios&url=${encodeURIComponent(url)}`]) {
              try {
                  const r = await axios.get(api, { timeout:15000 });
                  let vid = r.data?.result?.url || r.data?.url;
                  if(vid) { await dl(vid, out); const ctx=brand(config); await s.sendMessage(m.chat, { video:fs.readFileSync(out), caption:"✅ Downloaded from Twitter/X", contextInfo:ctx }, { quoted:m }); cleanup(out); return; }
              } catch {}
          }
          reply("❌ Download failed.");
      }
