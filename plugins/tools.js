// © 2026 Alpha - TOOLS (SELF-CONTAINED STICKER CONVERSION)
...
// ---------- CREATE WEBP STICKER FROM IMAGE/VIDEO BUFFER ----------
async function bufferToWebpSticker(mediaBuffer, isVideo) {
    ...
    try {
        ...
        const json = {
            ...
            "sticker-pack-name": "Alpha Bot",
            "sticker-pack-publisher": "Sticker Maker",
            "emojis": ["🤖"]
        };
...
module.exports = [
    ...
    // 5. STICKER MAKER (SELF-CONTAINED)
    {
        command: "sticker", aliases: ["s","st"], category: "tools",
        execute: async (s, m, { reply }) => {
            if (!m.quoted || !m.quoted.message) return reply("❌ Please *reply* to an image or video first!");
