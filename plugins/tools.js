...
const { writeExif } = require('../library/exif');   // your own rock‑solid converter
...
// helper: webp → png image buffer
async function webpToPng(webpBuffer) {
    const img = new webp.Image();
    await img.load(webpBuffer);
    // render the first frame to PNG
    const pngBuf = await img.toBuffer('image/png');
    return pngBuf;
}

module.exports = [
    // 0. DIAGNOSTIC (remove after testing)
    {
        command: "toolstest",
        aliases: ["ttest"],
        category: "tools",
        execute: async (s, m, { reply }) => {
            reply("✅ tools.js is active. Sticker/Toimg commands have been upgraded.");
        }
    },
    // 1. CALC
    ...
    // 5. STICKER MAKER (using your exif library)
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            try {
                ...
                // get raw media buffer
                let mediaBuffer;
                try {
                    mediaBuffer = await s.downloadMediaMessage(quoted);
                    console.log("✅ sticker: downloaded via downloadMediaMessage");
                } catch (e1) {
                    console.log("⚠️ sticker: fallback to downloadContentFromMessage");
                ...
                if (!mediaBuffer || mediaBuffer.length === 0) throw new Error("Empty media buffer");

                // convert to sticker webp using YOUR library
    ...
    // 6. STICKER TO IMAGE (using node-webpmux)
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            try {
                ...
                let webpBuffer;
                try {
                    webpBuffer = await s.downloadMediaMessage(quoted);
                    console.log("✅ toimg: downloaded via downloadMediaMessage");
                } catch (e1) {
                    console.log("⚠️ toimg: fallback to downloadContentFromMessage");
                ...
                if (!webpBuffer || webpBuffer.length === 0) throw new Error("Empty webp buffer");

                // convert webp to PNG
                let pngBuffer;
                try {
                    pngBuffer = await webpToPng(webpBuffer);
                } catch (webpErr) {
                    console.log("⚠️ webpToPng failed, sending original sticker as fallback:", webpErr.message);
                    // fallback: send the sticker again
                    await s.sendMessage(m.chat, { sticker: webpBuffer }, { quoted: m });
                    return reply("❌ Could not convert to image, here's the sticker instead.");
                }
