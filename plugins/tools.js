// © 2026 Alpha - TOOLS (STICKER/TOIMG FINAL FIX)
...
const { writeExif } = require('../library/exif');   // your own reliable converter
...
module.exports = [
    ...
    // 5. STICKER MAKER (FIXED – works when you quote an image/video)
    {
        command: "sticker", aliases: ["s","st"], category: "tools",
        execute: async (s, m, { reply }) => {
            // Must be a quoted message
            if (!m.quoted || !m.quoted.message) return reply("❌ Please *quote* (reply to) an image or video, then type .sticker");
            const q = m.quoted;
            const msgType = Object.keys(q.message)[0];
            if (msgType !== 'imageMessage' && msgType !== 'videoMessage') return reply("❌ The quoted message must be an image or a video!");

            try {
                reply("⏳ Cooking a sticker...");

                // Download the media using the exact same method as VV
                ...
                // Convert to sticker using your library (handles webp conversion + exif)
    ...
    // 6. STICKER TO IMAGE (FIXED)
    {
        command: "toimg", aliases: ["stickertoimg","simg"], category: "tools",
        execute: async (s, m, { reply }) => {
            if (!m.quoted || !m.quoted.message) return reply("❌ Please *quote* (reply to) a sticker, then type .toimg");
            const q = m.quoted;
            const msgType = Object.keys(q.message)[0];
            if (msgType !== 'stickerMessage') return reply("❌ The quoted message must be a sticker!");

            try {
                reply("⏳ Converting to image...");
                ...
                // Convert webp to PNG using node-webpmux
