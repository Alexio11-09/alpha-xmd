// © 2026 Alpha - TOOLS (WITH EXTREME LOGGING + FALLBACK)
...
module.exports = [
    ...
    // 5. STICKER MAKER (LOGGED + FALLBACK)
    {
        command: "sticker", aliases: ["s","st"], category: "tools",
        execute: async (s, m, { reply }) => {
            if (!m.quoted) return reply(guide("sticker", ".sticker (reply to an image/video)"));
            const quoted = m.quoted;
            console.log('📎 Sticker attempt on:', JSON.stringify(quoted.message?.[Object.keys(quoted.message||{})[0]]?.constructor.name));
            ...
            try {
                ...
                let buffer;
                // Try primary method: downloadMediaMessage
                try {
                    buffer = await s.downloadMediaMessage(quoted);
                    console.log('✅ Downloaded via downloadMediaMessage');
                } catch (e) {
                    console.log('⚠️ downloadMediaMessage failed, using downloadContentFromMessage', e.message);
                ...
                // Attempt to send as sticker
                try {
                    await s.sendMessage(m.chat, { sticker: buffer }, { quoted: m });
                    reply(success.sticker);
                } catch (stickerErr) {
                    console.log('❌ Sticker send failed, sending as image:', stickerErr.message);
                    // Fallback: send as image/video
                    if (msgType === 'imageMessage') {
                        await s.sendMessage(m.chat, { image: buffer, caption: "Couldn't make sticker, here's the image." }, { quoted: m });
                    } else {
                        await s.sendMessage(m.chat, { video: buffer, caption: "Couldn't make sticker, here's the video." }, { quoted: m });
                    }
                }
            } catch (err) {
                console.error("🔥 Sticker error:", err);
                reply("❌ Failed to create sticker. The media might be too large or corrupted.");
            }
        }
    },
    // 6. STICKER TO IMAGE (LOGGED + FALLBACK)
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            try {
                ...
                try {
                    buffer = await s.downloadMediaMessage(quoted);
                    console.log('✅ Downloaded sticker via downloadMediaMessage');
                } catch (e) {
                    console.log('⚠️ downloadMediaMessage failed, using downloadContentFromMessage', e.message);
                ...
                try {
                    ...
                    console.log('❌ Image send failed, trying as sticker again:', imgErr.message);
                    // Fallback: send as sticker itself (just to avoid silence)
                    await s.sendMessage(m.chat, { sticker: buffer }, { quoted: m });
                    reply("❌ Could not convert to image, but here's the sticker again.");
                }
            } catch (err) {
                console.error("🔥 Toimg error:", err);
                reply("❌ Failed to convert sticker. It might be animated or too large.");
            }
        }
    },
    // 7. GETPP
