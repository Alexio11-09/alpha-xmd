...
module.exports = [
    ...
    // 5. STICKER MAKER (FINAL FIX – using wa-sticker-formatter)
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            try {
                ...
                reply("❌ Failed to create sticker. The media might be too large or unsupported.");
            }
        }
    },
    // 6. STICKER TO IMAGE (FINAL FIX – using wa-sticker-formatter)
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            try {
                ...
                // toMessage() returns an image message object
                const imageMessage = await sticker.toMessage();
                // Extract image buffer from the object
                const imageBuffer = imageMessage.imageMessage?.jpegThumbnail || Buffer.from([]);
                // Actually, toMessage returns a full message, we can send it directly
