...
module.exports = [
    ...
    // ==================== 7. GET PROFILE PICTURE ====================
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            try {
                ...
                reply("👤 No profile picture found. This person is a ghost.");
            }
        }
    },
    ...
    // ==================== 12. LYRICS (RELIABLE API) ====================
    {
        ...
        execute: async (s, m, { args, reply }) => {
            ...
            try {
                // Primary API
                let res = await axios.get(`https://lyricsapi.fly.dev/api/lyrics?q=${encodeURIComponent(query)}`, { timeout: 10000 });
                let lyrics = res.data?.result?.lyrics;
                // Fallback to another API if first fails or returns nothing
                if (!lyrics) {
                    res = await axios.get(`https://api.lyrics.ovh/v1/${encodeURIComponent(query)}`, { timeout: 10000 });
                ...
                reply("❌ Lyrics not found. Maybe the song is too underground or the service is down.");
            }
        }
    },
    ...
    // ==================== 14. VIDEO TO MP3 (WORKING) ====================
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            reply(success.tomp3);
            const tempVideo = path.join(__dirname, '../temp', `video_${Date.now()}.mp4`);
            const tempAudio = path.join(__dirname, '../temp', `audio_${Date.now()}.mp3`);
            if (!fs.existsSync(path.dirname(tempVideo))) fs.mkdirSync(path.dirname(tempVideo), { recursive: true });
