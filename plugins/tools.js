...
let Sticker;
try { Sticker = require('wa-sticker-formatter').Sticker; } catch { /* will handle missing package */ }
...
module.exports = [
    ...
    // ==================== 11. EMOJIMIX (ULTRA-RELIABLE) ====================
    {
        ...
        execute: async (sock, m, { args, reply }) => {
            ...
            // Extract the first two emojis – works with or without spaces
            ...
            console.log(`Emojimix request: ${emoji1} + ${emoji2}`);

            // Primary API (very reliable)
            const apiUrl = `https://api.davidcyriltech.my.id/emojimix?emoji1=${encodeURIComponent(emoji1)}&emoji2=${encodeURIComponent(emoji2)}`;

            try {
                const res = await axios.get(apiUrl, { responseType: 'arraybuffer', timeout: 15000 });
                const buffer = Buffer.from(res.data);

                // Try to send as sticker, fall back to image if Sticker not available
                if (Sticker) {
                    try {
                        ...
                        return reply(emojimixSuccess[Math.floor(Math.random() * emojimixSuccess.length)]);
                    } catch (stickerErr) {
                        console.log("Sticker conversion failed, sending as image:", stickerErr.message);
                        // fallback to image
                    }
                }

                // Fallback: send as image
                await sock.sendMessage(m.chat, { image: buffer, caption: emojimixSuccess[Math.floor(Math.random() * emojimixSuccess.length)] }, { quoted: m });
            } catch (err) {
                console.error("Emojimix API error:", err.message);
