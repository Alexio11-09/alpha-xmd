// © 2026 Alpha - TOOLS (STRUCTURE DIAGNOSTIC)
...
module.exports = [
    ...
    // 5. STICKER DIAGNOSTIC – SHOWS RAW QUOTED MESSAGE STRUCTURE
    {
        ...
        execute: async (s, m, { reply }) => {
            ...
            const q = m.quoted;
            // Show the raw message keys
            const keys = Object.keys(q.message).join(", ");
            reply(`📋 Quoted message keys: ${keys}\n\nNow I'll show the full structure (truncated).`);
            try {
                const json = JSON.stringify(q.message, null, 2);
                const short = json.length > 2500 ? json.substring(0, 2500) + "..." : json;
                await s.sendMessage(m.chat, { text: short }, { quoted: m });
            } catch {
                reply("Could not stringify message object.");
            }
        }
    },
