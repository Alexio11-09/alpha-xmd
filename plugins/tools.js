// © 2026 Alpha - TOOLS (ULTIMATE DIAGNOSTIC)
...
module.exports = [
    ...
    // 5. STICKER MAKER (DIAGNOSTIC – reveals m object)
    {
        command: "sticker", aliases: ["s","st"], category: "tools",
        execute: async (s, m, { reply }) => {
            const q = m.quoted;
            const hasQuoted = !!q;
            const msgType = hasQuoted ? (Object.keys(q.message || {})[0] || 'no keys') : 'no quoted';
            const ctxQuoted = !!m.msg?.contextInfo?.quotedMessage;
            reply(`📋 Quoted object: ${hasQuoted}\nType: ${msgType}\nctxInfo.quotedMessage: ${ctxQuoted}\n\nWe'll now fix the sticker code.`);
        }
    },
    // 6. STICKER TO IMAGE (ORIGINAL – untouched)
    {
