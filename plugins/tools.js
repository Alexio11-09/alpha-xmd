...
module.exports = [
    ...
    // ==================== 7. GET PROFILE PICTURE (REPLY OR MENTION) ====================
    {
        command: "getpp", aliases: ["getprofile","pp"], category: "tools",
        execute: async (s, m, { reply }) => {
            // Priority: reply > mention > (if none, show guide)
            let target;
            if (m.quoted) {
                ...
                return reply(guide("getpp", "Reply to a user's message or mention them: .getpp @user"));
            }
            try {
                ...
                reply(`👤 No profile picture found for @${target.split('@')[0]}. They're a ghost.`, { mentions: [target] });
            }
        }
    },
