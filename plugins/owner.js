// © 2026 Alpha - OWNER COMMANDS (FULL + CHANNEL)
...
module.exports = [
    ...
    // ==================== GET CHANNEL JID ====================
    {
        ...
        execute: async (sock, m, { args, reply }) => {
            ...
            if (!inviteCode) return reply("❌ Invalid channel link!");
            
            try {
                const res = await sock.newsletterSubscribe(inviteCode);
                const jid = res.id + '@newsletter';
                reply(`✅ *Channel JID:*\n\n\`${jid}\`\n\n📌 Add this to your config.js!`);
            } catch (err) {
                reply(`❌ Failed to get channel JID:\n${err.message}`);
            }
        }
    },
        ...
        execute: async (sock, m, { reply }) => {
            try {
                ...
                if (!channels || channels.length === 0) {
                    return reply("❌ Bot is not subscribed to any channels!");
                }
                
                let text = `📰 *SUBSCRIBED CHANNELS*\n\n`;
                for (let ch of channels) {
                    text += `📌 *${ch.name}*\n`;
                    text += `🆔 \`${ch.id}@newsletter\`\n\n`;
                }
                
                reply(text);
            } catch (err) {
                reply(`❌ Failed to fetch channels:\n${err.message}`);
            }
        }
    }
